 ...
 *
 */
Class Manage_billet_model extends APP_Model {

    /**
     * 	Define table attributes
     */
    var $db_table = "billet_info";
    var $primary_key = "billet_info_id";
    ...
    /* 	
      # Function          :	manage_other_billet_live_search
    ...
    function manage_other_billet_live_search($searchdata, $keyword, $sort_column, $sort_order, $limit, $start) {
        
        // $first_name = $searchdata['first_name'];
        // $logged_users_id = $searchdata['logged_users_id'];

        $this->db->select("billet_info.*");
       // $this->db->select("users.id AS users_id");
        ...
        // }
        if ($keyword) {
            $this->db->where("(billet_info.billet_name LIKE '%$keyword%' OR  billet_info.description LIKE '%$keyword%')");
        }

        // $this->db->join('users_groups', 'users_groups.user_id = users.id', 'LEFT');
        // $this->db->join('groups', 'groups.id = users_groups.group_id', 'LEFT');

        $this->db->from('billet_info');
        // $this->db->order_by($sort_column, $sort_order);
        $this->db->limit($start, $limit);
         $this->db->group_by("billet_info.billet_info_id");
        $queryOrg = $this->db->get();
    ...
    /* 	
      # Function          :	manage_other_billet_live_search_count
      # Purpose           :	count for the keyword
      # params            :	keyword
      # Return            :   number
     */

    function manage_other_billet_live_search_count($searchdata, $keyword) {
    ...
    function insert_billet($data= array()) {
        // print_r($data);exit;
      return $this->db->insert('billet_info', $data);
    }
     
     function get_billet($data = array()) {
       //print_r($data);exit;
      $this->db->select("*");
      $this->db->where('billet_info_id', $data);
      $this->db->from('billet_info');
      $queryOrg = $this->db->get();
        if ($queryOrg->num_rows() > 0) {
            $rowcount = $queryOrg->num_rows();
  ...
  function update_billet($data=array(), $billet_info_id) {
    $this->db->where('billet_info_id', $billet_info_id);
    return $this->db->update('billet_info', $data);
        
  }

  function delete_billet($data) {
