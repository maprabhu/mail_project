 ...
 *
 */
Class Manage_power_model extends APP_Model {

    /**
     * 	Define table attributes
     */
    var $db_table = "power_info";
    var $primary_key = "power_info_id";
    ...
    function manage_other_power_live_search($searchdata, $keyword, $sort_column, $sort_order, $limit, $start) {
        
        // $first_name = $searchdata['first_name'];
        // $logged_users_id = $searchdata['logged_users_id'];

        $this->db->select("power_info.*");
       // $this->db->select("users.id AS users_id");
        ...
        // }
        if ($keyword) {
            $this->db->where("(power_info.opening_reading LIKE '%$keyword%' OR  power_info.closing_reading LIKE '%$keyword%')");
        }

        // $this->db->join('users_groups', 'users_groups.user_id = users.id', 'LEFT');
        // $this->db->join('groups', 'groups.id = users_groups.group_id', 'LEFT');

        $this->db->from('power_info');
        // $this->db->order_by($sort_column, $sort_order);
        $this->db->limit($start, $limit);
         $this->db->group_by("power_info.power_info_id");
        $queryOrg = $this->db->get();
    ...
    /* 	
      # Function          :	manage_other_power_live_search_count
      # Purpose           :	count for the keyword
      # params            :	keyword
      # Return            :   number
     */

    function manage_other_power_live_search_count($searchdata, $keyword) {
    ...
    function insert_power($data= array()) {
         // print_r($data);exit;
      return $this->db->insert('power_info', $data);
    }
     
     function get_power($data = array()) {
       //print_r($data);exit;
      $this->db->select("*");
      $this->db->where('power_info_id', $data);
      $this->db->from('power_info');
      $queryOrg = $this->db->get();
        if ($queryOrg->num_rows() > 0) {
            $rowcount = $queryOrg->num_rows();
  ...
  function update_power($data=array(), $power_id) {
    $this->db->where('power_info_id', $power_id);
    return $this->db->update('power_info', $data);
        
  }

  function delete_power($data) {
   // print_r($data);exit;
