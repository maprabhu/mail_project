...
class Ion_auth_model extends CI_Model {
     ...
     * @author Mathew
     * */
    public function register_api($identity, $password, $additional_data = array(), $groups = array()) {
        $this->trigger_events('pre_register');
        ...
        if ($this->identity_check($identity)) {
            //$this->set_error('account_creation_duplicate_identity');
            return "Duplicate account entry";
        } elseif (!$this->config->item('default_group', 'ion_auth') && empty($groups)) {
            //$this->set_error('account_creation_missing_default_group');
            return "Default group missing";
        }
    ...
    /**
     * get_mul_users_groups_onuser
     *
     * @return array
     * @author Ben Edmunds
     * */
    public function get_mul_users_groups_onuser($id = FALSE) {
        
        $this->db->select("users_groups.*",FALSE);
        //$this->db->join('users', 'users.id = marketing_lead_followups.attended_by_user_id', 'LEFT');
     ...
     * @author Phil Sturgeon
     * */
    public function update_api($id, array $data) {
        $this->trigger_events('pre_update_user');
