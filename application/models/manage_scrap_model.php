 ...
 *
 */
Class Manage_scrap_model extends APP_Model {

    /**
     * 	Define table attributes
     */
    var $db_table = "scrap_master";
    var $primary_key = "scrap_master_id";
    ...
    function manage_other_scrap_live_search($searchdata, $keyword, $sort_column, $sort_order, $limit, $start) {
        
        // $first_name = $searchdata['first_name'];
        // $logged_users_id = $searchdata['logged_users_id'];

        $this->db->select("scrap_master.*");
       // $this->db->select("users.id AS users_id");
        ...
        // }
        if ($keyword) {
            $this->db->where("(scrap_master.scrap_name LIKE '%$keyword%' OR  scrap_master.description LIKE '%$keyword%')");
        }

        // $this->db->join('users_groups', 'users_groups.user_id = users.id', 'LEFT');
        // $this->db->join('groups', 'groups.id = users_groups.group_id', 'LEFT');

        $this->db->from('scrap_master');
        // $this->db->order_by($sort_column, $sort_order);
        $this->db->limit($start, $limit);
         $this->db->group_by("scrap_master.scrap_master_id");
        $queryOrg = $this->db->get();
    ...
    /* 	
      # Function          :	manage_other_scrap_live_search_count
      # Purpose           :	count for the keyword
      # params            :	keyword
      # Return            :   number
     */

    function manage_other_scrap_live_search_count($searchdata, $keyword) {
    ...
    function insert_scrap($data= array()) {
        // print_r($data);exit;
      return $this->db->insert('scrap_master', $data);
    }
     
     function get_scrap($data = array()) {
       //print_r($data);exit;
      $this->db->select("*");
      $this->db->where('scrap_master_id', $data);
      $this->db->from('scrap_master');
      $queryOrg = $this->db->get();
        if ($queryOrg->num_rows() > 0) {
            $rowcount = $queryOrg->num_rows();
  ...
  function update_scrap($data=array(), $scrap_id) {
    $this->db->where('scrap_master_id', $scrap_id);
    return $this->db->update('scrap_master', $data);
        
  }

  function delete_scrap($data) {
