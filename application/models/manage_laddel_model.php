 ...
 *
 */
Class Manage_laddel_model extends APP_Model {

    /**
     * 	Define table attributes
     */
    var $db_table = "laddle_info";
    var $primary_key = "laddle_info_id";
    ...
    /* 	
      # Function          :	manage_other_laddel_live_search
    ...
    function manage_other_laddel_live_search($searchdata, $keyword, $sort_column, $sort_order, $limit, $start) {
        
        // $first_name = $searchdata['first_name'];
        // $logged_users_id = $searchdata['logged_users_id'];

        $this->db->select("laddle_info.*");
       // $this->db->select("users.id AS users_id");
        ...
        // }
        if ($keyword) {
            $this->db->where("(laddle_info.laddel_number LIKE '%$keyword%' OR  laddle_info.description LIKE '%$keyword%')");
        }

        // $this->db->join('users_groups', 'users_groups.user_id = users.id', 'LEFT');
        // $this->db->join('groups', 'groups.id = users_groups.group_id', 'LEFT');

        $this->db->from('laddle_info');
        // $this->db->order_by($sort_column, $sort_order);
        $this->db->limit($start, $limit);
         $this->db->group_by("laddle_info.laddle_info_id");
        $queryOrg = $this->db->get();
    ...
    /* 	
      # Function          :	manage_other_laddel_live_search_count
      # Purpose           :	count for the keyword
      # params            :	keyword
      # Return            :   number
     */

    function manage_other_laddel_live_search_count($searchdata, $keyword) {
    ...
    function insert_laddle($data= array()) {
      //print_r($data);exit;
      return $this->db->insert('laddle_info', $data);
   }
     
     function get_laddle($data = array()) {
       //print_r($data);exit;
      $this->db->select("*");
      $this->db->where('laddle_info_id', $data);
      $this->db->from('laddle_info');
      $queryOrg = $this->db->get();
        if ($queryOrg->num_rows() > 0) {
            $rowcount = $queryOrg->num_rows();
  ...
  function update_laddle($data=array(), $laddle_info_id) {
    // print_r($data);
    // echo $laddle_info_id;exit; 
    $this->db->where('laddle_info_id', $laddle_info_id);
    return $this->db->update('laddle_info', $data);

        
  }

  function delete_laddle($data) {
