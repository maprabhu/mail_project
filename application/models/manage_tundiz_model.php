 ...
 *
 */
Class Manage_tundiz_model extends APP_Model {

    /**
     * 	Define table attributes
     */
    var $db_table = "tundiz_info";
    var $primary_key = "tundiz_info_id";
    ...
    function manage_other_tundiz_live_search($searchdata, $keyword, $sort_column, $sort_order, $limit, $start) {
        
        // $first_name = $searchdata['first_name'];
        // $logged_users_id = $searchdata['logged_users_id'];

        $this->db->select("tundiz_info.*");
       // $this->db->select("users.id AS users_id");
        ...
        // }
        if ($keyword) {
            $this->db->where("(tundiz_info.tundiz_number LIKE '%$keyword%' OR  tundiz_info.description LIKE '%$keyword%')");
        }

        // $this->db->join('users_groups', 'users_groups.user_id = users.id', 'LEFT');
        // $this->db->join('groups', 'groups.id = users_groups.group_id', 'LEFT');

        $this->db->from('tundiz_info');
        // $this->db->order_by($sort_column, $sort_order);
        $this->db->limit($start, $limit);
         $this->db->group_by("tundiz_info.tundiz_info_id");
        $queryOrg = $this->db->get();
    ...
    /* 	
      # Function          :	manage_other_tundiz_live_search_count
      # Purpose           :	count for the keyword
      # params            :	keyword
      # Return            :   number
     */

    function manage_other_tundiz_live_search_count($searchdata, $keyword) {
    ...
    function insert_tundiz($data= array()) {
      return $this->db->insert('tundiz_info', $data);
    }
     
     function get_tundiz($data = array()) {
       //print_r($data);exit;
      $this->db->select("*");
      $this->db->where('tundiz_info_id', $data);
      $this->db->from('tundiz_info');
      $queryOrg = $this->db->get();
        if ($queryOrg->num_rows() > 0) {
            $rowcount = $queryOrg->num_rows();
  ...
  function update_tundiz($data=array(), $tundiz_id) {
    $this->db->where('tundiz_info_id', $tundiz_id);
    return $this->db->update('tundiz_info', $data);
        
  }

  function delete_tundiz($data) {
