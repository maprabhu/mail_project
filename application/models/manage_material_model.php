 ...
 *
 */
Class Manage_material_model extends APP_Model {

    /**
     * 	Define table attributes
     */
    var $db_table = "material";
    var $primary_key = "material_id";
    ...
    function manage_other_material_live_search($searchdata, $keyword, $sort_column, $sort_order, $limit, $start) {
       ...
       // $this->db->select("users.id AS users_id");
        ...
        // }
        if ($keyword) {
            $this->db->where("(material.name LIKE '%$keyword%' OR  material.description LIKE '%$keyword%')");
        }
        ...
        // $this->db->order_by($sort_column, $sort_order);
        $this->db->limit($start, $limit);
         $this->db->group_by("material.material_id");
        $queryOrg = $this->db->get();
    ...
    /* 	
      # Function          :	manage_other_material_live_search_count
      # Purpose           :	count for the keyword
      # params            :	keyword
      # Return            :   number
     */

    function manage_other_material_live_search_count($searchdata, $keyword) {
    ...
    function insert_material($data= array()) {
      //print_r($data);exit;
      return $this->db->insert('material', $data);

    }
     
     function get_material($data = array()) {
       //print_r($data);exit;
      $this->db->select("*");
      $this->db->where('material_id', $data);
      $this->db->from('material');
      $queryOrg = $this->db->get();
        if ($queryOrg->num_rows() > 0) {
            $rowcount = $queryOrg->num_rows();
  ...
  function update_material($data=array(), $material_id) {
